...
void menu::buildmenu(char nodeIndex, Adafruit_SSD1327 *Disp)
{
  ...
  Disp->setTextSize(2); // Normal 1:1 pixel scale
  // Menu title front decorator
  Disp->write("-");
  // Display menu name
  Disp->write(this->name);
...
void menuFrame::build(Adafruit_SSD1327 *Disp)
{
  ...
  menuList[currentMenu].buildmenu(this->nodeIndex, Disp);
}
...
/*---------------------------------------------------------------------------------------------------------------------
| oledSystemInit; This function initializes all of the OLED specific components and then clears the OLED_Display.
